...
def welcomepic(pic, user, chatname, id, uname, brightness_factor=1.3):
    background = Image.open("ANNIEMUSIC/assets/annie/heartbeat_wel.png")
    pfp = Image.open(pic).convert("RGBA")
    pfp = circle(pfp)
    pfp = pfp.resize((830, 840))
    draw = ImageDraw.Draw(background)
    font_large = ImageFont.truetype('ANNIEMUSIC/assets/annie/ArialReg.ttf', size=130)
    font_small = ImageFont.truetype('ANNIEMUSIC/assets/annie/ArialReg.ttf', size=130)
    draw.text((1820, 1510), f'{user}', fill=(201, 2, 2), font=font_large)
    draw.text((1620, 1510), f'{id}', fill=(201, 2, 2), font=font_large)
